...
editor.init = () => {
  ...
  */
  ...
  const serverFileUpload=function(){
	...
	const uploadHandler=async function(wind,svg){	
		...
		min64= 'data:image/svg+xml,'+min64;		
		editor.showSaveWarning = false;
  ...
  */
  ...
  svgCanvas.bind('uploaded', uploadHandler);// ! 上传
  ...
  */
  ...
  const clickUpload = function () {
    ...
    svgCanvas.upload(saveOpts);	
  };
  const clickUploadPrefs = function () {
	clickServerRefresh();
    showServerFileBox();
  };  
  let loadingURL;
  ...
  */
  ...
  const clickServerRefresh = function () {
	  console.log('clickServerRefresh');
	  $.get('/upload/svg.list.php',function(e){
			var str='<ul>';
			for(var i=0;i<e.length;i++){
			
			str+='<li data-src="/upload/svg/'+e[i]+'">'+e[i]+'</li>';
			
			}
			str+='</ul>';
			//$('#server_file_list_content').empty();
			$('#server_file_list_content').empty().html(str);
			$('#server_file_list_content ul li').click(function(){
				
				
			});
			
			$('#server_file_list_content li').click(function(){
				showSvgViewer();
				$(this).css("background","#FFFFa3dd").siblings().css("background",""); 			
				var time=new Date().getTime();
				$('#svg_viewer_svg_content').html($(this).text()+'<img style="width:90%;width:90%" src="'+$(this).attr('data-src')+'?t='+time+'" />');
				$('#server_file_file_name').val($(this).text());
			});
		}).fail(function(e){
				$.alert('Error:'+e.responseText);
			});
  };
 const clickServerOpen = async function () {
	  console.log('clickServerOpen');
	 const ok = await editor.openPrep();
     if (!ok) { return; }
	 
      //svgCanvas.clear();
	  const name=$('#server_file_file_name').val();
	  const url='/upload/svg/'+name;
	  
	  setServerFilePath(url,name);
		hideSvgViewer();
  ...
  */
  ...
  $('#svg_docprops_container, #svg_prefs_container,#server_file_container,#svg_viewer_container').draggable({
  ...
  */
  const showDocProperties = function () {
	...
	const showServerFileBox=function(){
		$('#server_file_file_name').val(editor.serverFileName);
		 $('#server_file_box').show();
	};
	const showSvgViewer=function(){
		 $('#svg_viewer_box').show();
	};
  ...
  */
  const hideDocProperties = function () {
	...
	const hideServerFileBox = function () {
		$('#server_file_box').hide();
	};
	const hideSvgViewer = function () {
		$('#svg_viewer_box').hide();
	};
  /**
  *
  ...
  */
  ...
  if (isMac() && !window.opera) {
    const shortcutButtons = [
      'tool_clear', 'tool_save', 'tool_upload', 'tool_source',
  ...
  */
  ...
  const dialogSelectors = [
    '#tool_source_cancel', '#tool_docprops_cancel','#server_file_cancel','#svg_viewer_cancel',
    ...
    */
    ...
    const toolButtons = [
	  ...
	  {sel: '#tool_upload', fn () {
		...
		if (editingsource) {
           //saveSourceEditor();
		   console.log('#tool_upload saveSourceEditor');
        } else {
			console.log('#tool_upload  clickSave');
          clickUpload();
        }
      }, evt: 'mouseup'},
	  {sel: '#tool_upload_prefs', fn: clickUploadPrefs, evt: 'mouseup'},
      ...
      {sel: '#tool_source_save', fn: saveSourceEditor, evt: 'click'},
	   {sel: '#tool_source_cancel', fn:  hideSourceEditor, evt: 'click'},
      {sel: '#tool_docprops_save', fn: saveDocProperties, evt: 'click'},
	  {sel: '#tool_docprops_cancel', fn: hideDocProperties, evt: 'click'},
	    {sel: '#server_file_cancel', fn: hideServerFileBox, evt: 'click'},
		 {sel: '#server_file_upload', fn: clickUpload, evt: 'click'},
 {sel: '#server_file_open', fn: clickServerOpen, evt: 'click'},		
{sel: '#server_file_refresh', fn: clickServerRefresh, evt: 'click'},	
{sel: '#svg_viewer_cancel', fn: hideSvgViewer, evt: 'click'}, 
      {sel: '#tool_docprops', fn: showDocProperties, evt: 'click'},
      {sel: '#tool_prefs_save', fn: savePreferences, evt: 'click'},
	   {sel: '#tool_prefs_cancel', fn:hidePreferences, evt: 'click'},
      ...
      setAll () {
        ...
        toolButtons.forEach((opts) => {
          ...
          if (opts.key) {
			...
			keyHandler[key.toLowerCase()] = {fn, pd}; });
